...
"2014AT16RFOP001" AS (
  SELECT
    ...
    (unionskofinanzierungssatz_pro_prioritatsachse_union_cofinancin::DECIMAL)*100.0 AS eu_cofinancing_rate,
...
"2014BE16RFOP001_1" AS (
  SELECT
    ...
    union_cofinancing_rate_as_per_priority_axes_::DECIMAL AS eu_cofinancing_rate,
    country AS country,
...
"2014BE16RFOP001_2" AS (
  SELECT
    ...
    beneficiary_name AS beneficiary_name,
    operation_summary AS operation_summary,
    TO_DATE("operation_start_date", 'DD/MM/YYYY') AS operation_start_date,
    TO_DATE("operation_end_date", 'DD/MM/YYYY') AS operation_end_date,
    'EUR' AS currency,
    REPLACE(total_eligible_expenditure_allocated_to_the_operation_in_euros,' ','')::DECIMAL AS operation_total_expenditure,
    REPLACE(union_cofinancing_rate_as_per_priority_axes_,' ','')::DECIMAL AS eu_cofinancing_rate,
    country AS country,
    operation_postcode::DECIMAL::INTEGER::TEXT AS operation_location,
    REPLACE(code_of_category_of_intervention_of_the_operation,'.0','') AS code_of_category_intervention,
    name_of_category_of_intervention_of_the_operation AS name_of_category_intervention,
...
"2014BE16RFOP003_1" AS (
  SELECT
    ...
    TO_DATE("date_debut_de_loperation_operation_start_date", 'DD/MM/YYYY') AS operation_start_date,
    TO_DATE("date_de_fin_de_loperation_operation_end_date", 'DD/MM/YYYY') AS operation_end_date,
    'EUR' AS currency,
...
"2014BG05M2OP001_1" AS (
  SELECT
    ...
    _of_ec_cofinancing::DECIMAL AS eu_cofinancing_rate,
    NULL AS country,
...
"2014CZ_1" AS (
  SELECT
    ...
    mira_spolufinancovani_ze_strany_unie::DECIMAL AS eu_cofinancing_rate,
    zeme AS country,
